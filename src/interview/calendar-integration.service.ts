...
export interface CalendarProvider {
  ...
  getAvailability(email: string, startDate: Date, endDate: Date): Promise<AvailabilitySlot[]>;
}
...
export class GoogleCalendarProvider implements CalendarProvider {
  ...
  async updateEvent(eventId: string, event: CalendarEvent): Promise<void> {
    this.logger.log(`Updating Google Calendar event ${eventId}: ${event.summary}`);
  ...
  async getAvailability(email: string, startDate: Date, endDate: Date): Promise<AvailabilitySlot[]> {
    this.logger.log(`Getting availability for ${email} from ${startDate} to ${endDate}`);
...
export class OutlookCalendarProvider implements CalendarProvider {
  ...
  async updateEvent(eventId: string, event: CalendarEvent): Promise<void> {
    this.logger.log(`Updating Outlook Calendar event ${eventId}: ${event.summary}`);
...
export class CalendarIntegrationService {
  ...
  async createCalendarEvent(interview: Interview & { 
    ...
    try {
      ...
      this.logger.log(`Created calendar event ${eventId} for interview ${interview.id}`);
      return eventId;
    } catch (error) {
      this.logger.error(`Failed to create calendar event for interview ${interview.id}:`, error);
      throw new BadRequestException('Failed to create calendar event');
    }
  }

  async updateCalendarEvent(eventId: string, interview: Interview & { 
    candidate: { user: { name: string; email: string } }; 
    interviewers: Array<{ user: { name: string; email: string } }> 
  }): Promise<void> {
    const provider = this.getDefaultProvider();
    ...
    try {
      ...
      this.logger.log(`Updated calendar event ${eventId} for interview ${interview.id}`);
    } catch (error) {
      this.logger.error(`Failed to update calendar event ${eventId} for interview ${interview.id}:`, error);
  ...
  async getAvailability(userId: string, startDate: Date, endDate: Date): Promise<AvailabilitySlot[]> {
    ...
    try {
      const availability = await provider.getAvailability(userEmail, startDate, endDate);
      this.logger.log(`Retrieved availability for user ${userId}: ${availability.length} slots`);
      return availability;
    } catch (error) {
      this.logger.error(`Failed to get availability for user ${userId}:`, error);
      throw new BadRequestException('Failed to retrieve calendar availability');
    }
  }

  async checkAvailability(userIds: string[], startTime: Date, endTime: Date): Promise<{ [userId: string]: boolean }> {
    ...
    for (const userId of userIds) {
      try {
        ...
        this.logger.error(`Failed to check availability for user ${userId}:`, error);
  ...
  private getDefaultProvider(): CalendarProvider {
    ...
    const defaultProvider = this.configService.get<string>('CALENDAR_PROVIDER', 'google');
    
    const provider = this.providers.get(defaultProvider);
    if (!provider) {
      throw new BadRequestException(`Unsupported calendar provider: ${defaultProvider}`);
    }
  ...
  async handleAuthCallback(code: string, state: string): Promise<{ provider: string; userId: string; success: boolean }> {
    ...
    try {
      ...
      this.logger.log(`Successfully authenticated ${provider} calendar for user ${userId}`);
      ...
      this.logger.error(`Failed to authenticate ${provider} calendar for user ${userId}:`, error);
